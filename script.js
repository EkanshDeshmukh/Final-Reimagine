...
const sectionListItem = section.querySelectorAll('.section-list-item');
const sectionListItemText = section.querySelectorAll('.section-list-item > h1');
const sectionListFigure = section.querySelectorAll('.section-list-item-figure');
const sectionMedia = section.querySelectorAll('.section-media');
...
// Set initial clip path for all sectionMedia elements
gsap.set(sectionMedia, { clipPath: clipPath.top });

const initAnimation = () => {
    // Set initial y position for text elements to be off-screen
    ...
    // Slide text into view and expand figures
    tlText.to(sectionListItemText, {
        y: '0',
        stagger: 0.032,
    })
    .from(sectionListFigure, {
        width: 0,
        stagger: 0.032
    }, 0.8);
...
const addEventListener = () => {
    sectionListItem.forEach((item, index) => {
        ...
        item.addEventListener('mouseenter', () => {
            gsap.timeline({ defaults: { duration: 0.64, ease: 'ease.inOut', overwrite: true } })
...
// Initialize the animation on page load or when ready
initAnimation();
